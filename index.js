...
longestSubstring = (arr) => {
  ...
  const permutator = (inputArr) => {
    ...
    const permute = (arg, m = []) => {
      ...
      if (arg.length === 0) {
        result.push(m); // m holds the elements -> as it goes through the loop it removes an element until it hits 0
      } else {
        for (let i = 0; i < arg.length; i++) {
          let curr = arg.slice();
          let next = curr.splice(i, 1);
          permute(curr.slice(), m.concat(next)); // passes a copy of curr, concats next into m -> pushes it into the end of the array -> repeats the loop until it hits 0
        }
      }
    };
